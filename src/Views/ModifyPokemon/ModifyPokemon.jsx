...
function ModifyPokemon() {
  ...
  useEffect(() => {
    fetch(`https://pokedex-back-deploy-pokedex.up.railway.app/pokemons/${id}`)
  ...
  return (
    <div className={styles.container}>
      ...
      >
        ...
        <Typechecked
          inputChecked={pokemonData.types}
          errors={errors}
          allTypes={allTypes}
          setPokemonData={setPokemonData}
          pokemonData={pokemonData}
        ...
        !errors.types ? (
          ...
          <button type="submit" className={styles.buttonCreate} disabled>
            Modify
          </button>
        )}
      </form>
      {pokemonCreated.exists && (
        <div className={styles.errorMessage}>Pokemon already exists</div>
      )}
      {pokemonCreated.creating && (
        <div className={styles.message}>Modifying...</div>
      )}
      {pokemonCreated.created && (
        <Link className={styles.linkDetail} to={`/detail/${pokemonCreated.id}`}>
          View created Pokemon
        </Link>
      )}
    </div>
  );
